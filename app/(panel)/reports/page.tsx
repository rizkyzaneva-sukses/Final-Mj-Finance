...
export default async function ReportsPage({ searchParams }: { searchParams: Params }) {
  ...
  return <div className="page-stack">
    ...
    <section className="panel table-panel"><div className="panel-title"><div><span className="eyebrow">REKAP UTAMA</span><h2>Arus kas per kementerian</h2></div></div>{ministryRows.length ? <div className="responsive-table"><table className="report-table responsive-report-table"><thead><tr><th>Kode</th><th>Kementerian</th><th>Pemasukan</th><th>Pengeluaran</th><th>Arus bersih</th></tr></thead><tbody>{ministryRows.map((row) => <tr key={row.code}><td data-label="Kode"><span className="ministry-code">{row.code}</span></td><td data-label="Kementerian"><strong>{row.ministry}</strong></td><td className="money-in" data-label="Pemasukan">{rupiah.format(row.income)}</td><td className="money-out" data-label="Pengeluaran">{rupiah.format(row.expense)}</td><td data-label="Arus bersih"><strong>{rupiah.format(row.net)}</strong></td></tr>)}</tbody></table></div> : <Empty />}</section>
